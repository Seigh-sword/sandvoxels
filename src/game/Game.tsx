...
import { PixelCube, PixelIcon } from '../ui/PixelIcon';
import { VoxelEngine, type GameStats, type SavedWorld } from './engine';
...
export default function Game({ world, settings, onExit }: Props) {
  ...
  return <div className="game-shell">
    ...
    <div className="game-topbar">
      <div className="game-world-info"><span className="small-brand"><PixelCube size={20} /></span><div><strong>{world.name}</strong><span><i className="status-dot" /> {world.mode} / {world.biome}</span></div></div>
    ...
    {paused && <div className="game-pause-overlay"><div className="pause-card">
      ...
      <p>{error || (started ? 'Your world will be right here when you are ready.' : 'Wander a little. Build a lot. Make this place your own.')}</p>
      {showHelp ? <div className="pause-controls">
        ...
        {[['W A S D / arrows', 'Move around'], ['Mouse / drag', 'Look around'], ['Space', 'Jump / fly up'], ['Left click / R', 'Mine a block'], ['Right click / E', 'Place a block'], ['1-9 / scroll', 'Choose a block'], ['F / Q', 'Fly / descend (Creative)'], ['Esc', 'Pause your world']].map(([key, text]) => <div key={key}><kbd>{key}</kbd><span>{text}</span></div>)}
      </div> : !error && <>
