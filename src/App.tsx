...
import { BIOMES, biomeId, biomeInfo, defaultSettings, heroImage, initialWorlds, loadLocal, newId, saveLocal, storageKey, type Biome, type GameMode, type Settings, type World } from './game/types';
...
export default function App() {
  ...
  const importWorld = async (file?: File) => {
    ...
    try {
      ...
      if (!data.world || !BIOMES.includes(data.world.biome) || !['Creative', 'Explorer'].includes(data.world.mode) || typeof data.world.name !== 'string' || !Number.isFinite(data.world.seed)) throw new Error('Invalid world');
      ...
      const editsXZ: unknown[] = Array.isArray(data.save?.editsXZ) ? data.save.editsXZ : [];
      if (keys.length > 200000 || keys.length !== values.length) throw new Error('Invalid blocks');
      if (editsXZ.length > 200000) throw new Error('Invalid blocks');
      for (let i = 0; i < keys.length; i++) {
        const key = keys[i];
        const value = values[i];
        if (typeof key !== 'number' || !Number.isInteger(key) || typeof value !== 'number' || value < 0 || value > 14) throw new Error('Invalid blocks');
      }
      const id = newId();
      if (!saveLocal(storageKey(`world-${id}`), { ...data.save, keys, values, position: data.save?.position ?? null, rotation: data.save?.rotation ?? null, flying: !!data.save?.flying })) throw new Error('Storage full');
  ...
  return <div className="app-shell">
    ...
    <aside className={`sidebar ${mobileMenu ? 'mobile-open' : ''}`}>
      ...
      <div className="sidebar-bottom">
        ...
        <button className="version-button" onClick={() => setDialog('updates')}><span><i className="status-dot" />Version 1.2.0</span><span className="version-label">What is new <PixelIcon name="arrowRight" size={12} /></span></button>
    ...
    <div className="main-shell">
      ...
      <main className="main-content">
        ...
        {page === 'Discover' && <>
          <div className="discover-intro"><PixelIcon name="compass" size={22} /><p>Nine handcrafted biomes, one endless world. Pick a home landscape and let the zones blend as you travel.</p><span>{BIOMES.length} biomes to call home</span></div>
          <div className="discover-grid">{BIOMES.map(item => <article className="discover-card" key={item}><div className="discover-image"><span className="biome-swatch tall" style={{ '--chip-top': biomeInfo[item].top, '--chip-side': biomeInfo[item].side } as React.CSSProperties}><PixelIcon name={biomeInfo[item].icon} size={30} /></span><span><PixelIcon name={biomeInfo[item].icon} size={14} />{item}</span></div><div className="discover-card-content"><span className="eyebrow">BIOME {biomeId[item] + 1} / 9</span><h2>{item}.</h2><p>{biomeInfo[item].blurb}</p><button className="secondary-button full-width" onClick={() => openCreate(item)}>Make this world yours<PixelIcon name="arrowRight" size={15} /></button></div></article>)}
          </div>
